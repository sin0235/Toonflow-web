                  ...
                  <t-checkbox-group
                    ...
                    <template v-for="opt in referenceOptions" :key="opt.value">
                      ...
                      <t-input-number
                        ...
                        size="small"
                        style="width: 80px"
                        :placeholder="$t('legacy.vendorCount')" />
                    </template>
                  </t-checkbox-group>
                </div>
              </div>
    ...
    <t-dialog
      ...
      <div class="data">
        <t-radio-group variant="default-filled" v-model="addMode">
          <t-radio-button value="importAdd">{{ $t("settings.vendor.importFile") }}</t-radio-button>
          <t-radio-button value="linkAdd">{{ $t("legacy.addByLink") }}</t-radio-button>
          <t-radio-button value="codeAdd">{{ $t("legacy.addByCode") }}</t-radio-button>
        </t-radio-group>
        <div class="linkAdd" v-if="addMode == 'linkAdd'">
          <t-alert theme="warning" style="margin-bottom: 20px">
            {{ $t("legacy.vendorLinkHelp") }}
          </t-alert>
...
async function handleConfirmModel() {
  ...
  if (editingModelIndex.value === null) {
    try {
      ...
      window.$message.error(err.message ?? $t("legacy.operationFailed"));
    }
    return;
  }
  if (editingModelIndex.value !== null) {
            ...
            if (!data.includes("vendor")) {
              ...
              if (data.includes("<html>")) {
                alertBox = DialogPlugin.alert({
                  theme: "danger",
                  header: $t("legacy.linkReturnedHtml"),
                  body: $t("legacy.proxyWarning"),
                  onConfirm: ({ e }) => {
                    alertBox.hide();
                  },
                });
